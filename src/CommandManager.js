...
class CommandManager {
	...
	callCommand(line, scope = 'SYSTEM') {
		...
		if (this.commands.has(command)) backmessage = this.commands.get(command).callback(command, line.split(' '), scope);
		if (backmessage) {
	...
	initializeDefaultCommands() {
		this.registerCommand(
			new Command('help', 'help', 'Description', (command, args, sender) => {
				console.log('------------------- HELP -------------------');
				console.log(' ');
				this.getAllCommandsWithoutAliases().forEach((command) => {
					console.log(
						'=> ' +
							(Array.isArray(command.command) ? command.command.join(', ') : command.command) +
							' : ' +
							command.usage +
							' : ' +
							command.description
		...
		this.registerCommand(
			new Command(['clear', 'c'], 'Clear', 'Clears the complete Node console!', (command, [...args], scope) => {
				console.log('\x1Bc');
			})
		);
	}
	...
	fixStdoutFor(cli) {
		...
		newStdout.write = function () {
			...
			const result = oldStdout.write.apply(this, Array.prototype.slice.call(arguments));
			cli._refreshLine();
			return result;
		};
	...
	registerCommand(command) {
		if (typeof command.command === 'string') this.commands.set(command.command.toLowerCase(), command);
		if (Array.isArray(command.command)) command.command.forEach((commands) => this.commands.set(commands.toLowerCase(), command));
	}
	/**
